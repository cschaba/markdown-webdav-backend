...
//go:embed templates/*.html static/style.css static/graph.js
var assets embed.FS
...
func (h *Handler) vault(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.root.Stat(rel + ".md"); err == nil {
...
func (h *Handler) crumbs(rel string) []crumb {
	...
	for i, part := range parts {
		out = append(out, crumb{Name: part, URL: h.Index.URL(strings.Join(parts[:i+1], "/"))})
	}
	return out
}
...
func (h *Handler) note(w http.ResponseWriter, r *http.Request, rel string) {
	...
	body := noteBody{HTML: html, Backlinks: h.Index.Backlinks(rel)}
...
func (h *Handler) drawing(w http.ResponseWriter, r *http.Request, rel string) {
	body := drawingBody{Backlinks: h.Index.Backlinks(rel)}
